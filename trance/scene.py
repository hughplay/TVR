...
class Scene:
    ...
    def perturb_camera(self, jitter=0.5):
        for key, camera in sorted(self.cameras.items()):
            for i in range(3):
                camera.location[i] = (
                    self.cameras_init_loc[key][i]
                    + (np.random.random() - 0.5) * 2.0 * jitter
                )
        self.view_directions = self._compute_directions()

    def perturb_lamp(self, jitter=1.0):
        for key, lamp in sorted(self.lamps.items()):
            for i in range(3):
                lamp.location[i] = (
                    self.lamps_init_loc[key][i]
                    + (random.random() - 0.5) * 2.0 * jitter
                )
    ...
    def add_object(
        self, shape, size, theta, x, y, material, color, name=None, record=True
    ):
    ...
    def replace_object(self, old_obj, new_obj, new_obj_desc):
        assert old_obj in self.objects, "{} is not in objects.".format(
            str(old_obj)
        )
        ...
        if old_obj in self.visible_objects:
            self.visible_objects[self.visible_objects.index(old_obj)] = new_obj
        if old_obj in self.invisible_objects:
            self.invisible_objects[
                self.invisible_objects.index(old_obj)
            ] = new_obj
    ...
    def set_position(self, obj, x, y, record=True):
        assert self.option.is_position_valid(
            x, y
        ...
        if not self.creating:
            ...
            self.modify_desc(obj, "view_coords", self.get_view_coords(obj))
    ...
    def is_overlapped(self, obj):
        ...
        for other_obj in self.objects:
            ...
            r_ = self.option.get_real_size(self.get_size(other_obj))
    ...
    def render(self, image_path, camera=None, width=320, height=240):
        ...
        if camera is not None:
            assert camera in self.cameras.values(), "Unknown camera: {}".format(
                camera
            )
        ...
        for camera_name, directions in self.view_directions.items():
            ...
            for name in ["left", "right", "front", "behind"]:
                ...
                for current_obj in self.visible_objects:
                    ...
                    relationships[name][str(current_idx)] = sorted(
                        list(related)
                    )
    ...
    def describe_by_index(self, idx):
        assert idx in range(
            len(self.objects)
        ), "index ({}) exceed range (0-{})".format(idx, len(self.objects) - 1)
        return self.objects_description[idx]
    ...
    def get_name(self, obj):
        if obj in self.objects:
            ...
            assert name == obj.name, "Name is conflict: {} vs. {}".format(
                name, obj.name
            )
        return obj.name
