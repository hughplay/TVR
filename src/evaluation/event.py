...
__all__ = [
    'EventEvaluator', 'LogEventEvaluator', 'JointEventEvaluator',
    'ReinforceEventEvaluator']
...
class LogEventEvaluator(EventEvaluator):
    def compute_loss(self, obj_choice, pair_choice, obj_target, pair_target):
        B, L, _ = obj_choice.shape
        loss_obj = F.nll_loss(
            obj_choice.view(B * L, -1).log(), obj_target.view(B * L),
            ignore_index=self.t.OBJ_PAD)
        loss_pair = F.nll_loss(
            pair_choice.view(B * L, -1).log(), pair_target.view(B * L),
            ignore_index=self.t.PAIR_PAD)
        loss = loss_obj + loss_pair
        return loss, loss_obj, loss_pair


class JointEventEvaluator(EventEvaluator):

    def map_choice(self, obj_choice, pair_choice):
        prob = torch.bmm(
            obj_choice[:, :self.t.NUMOBJECTS].unsqueeze(2),
            pair_choice[:, :self.t.n_pairs].unsqueeze(1)).flatten(1)
        choice = torch.cat(
            [prob, 1 - prob.sum(dim=1, keepdim=True)], dim=1).log()
        return choice

    def map_target(self, obj_target, pair_target):
        valid_pos = (obj_target < self.t.OBJ_EOS)
        ignore_index = self.t.NUMOBJECTS * self.t.n_pairs
        target =  valid_pos * (obj_target * self.t.n_pairs + pair_target) + (
            ~valid_pos) * ignore_index
        return target, ignore_index

    def compute_loss(self, obj_choice, pair_choice, obj_target, pair_target):
        B, L, _ = obj_choice.shape
        choice = self.map_choice(
            obj_choice.view(B * L, -1), pair_choice.view(B * L, -1))
        target, ignore_index = self.map_target(
            obj_target.view(B * L), pair_target.view(B * L))
        loss = F.nll_loss(choice, target, ignore_index=ignore_index)
